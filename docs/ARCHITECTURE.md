...
| `core/library` (`plinth-library`) | Модель библиотеки (`Track` → `Version` → `Source`, артисты, альбомы, плейлисты, прослушивания, склейки) и хранилище на SQLite: схема, репозитории, выборки для экранов, проверка целостности | Знать об FFI и Kotlin; ссылаться из проекции журнала на каталог внешними ключами |
