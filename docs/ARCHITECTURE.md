...
| `core/library` (`plinth-library`) | Модель библиотеки (`Track` → `Version` → `Source`, артисты, альбомы, плейлисты, прослушивания, склейки) и хранилище на SQLite: миграции только вперёд с копией перед каждой ([ADR 0011](adr/0011-migration-strategy.md)), репозитории, выборки для экранов, проверка целостности; сканер `scan/` — обход томов по прямым путям, сравнение с каталогом по времени и размеру файла, запись пачками | Знать об FFI и Kotlin; ссылаться из проекции журнала на каталог внешними ключами |
