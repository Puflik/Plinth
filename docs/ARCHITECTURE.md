...
ExoPlayer разрешает обращаться к себе только из одного потока, а команды
движка приходят из UI, службы и тестов. Поэтому у каждого `Media3Engine`
свой `HandlerThread`, в котором живут плеер и `PlayerListenerAdapter`.
Команда проверяет аргументы и состояние в потоке вызывающего — так контракт
требует бросать исключения, — и уходит в поток плеера сообщением. Обратно
плеер отвечает через `state`/`events`, которые безопасно читать откуда угодно.
...
| `Media3Engine` | Реализация `AudioEngine`: поток, проверки, публикация |
...
Каркас (эпик A), абстракция движка (B1) и `Media3Engine` (B2 без прогона
семи форматов и gapless). Дальше по вертикали «звук из файла»: фоновая
служба и `PlaybackController` (B3), тонкий UI.
