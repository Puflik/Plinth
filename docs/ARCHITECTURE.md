...
| `core/library` (`plinth-library`) | Модель библиотеки (`Track` → `Version` → `Source`, артисты, альбомы, плейлисты, прослушивания, склейки) и хранилище на SQLite: миграции только вперёд с копией перед каждой ([ADR 0011](adr/0011-migration-strategy.md)), репозитории, выборки для экранов, проверка целостности; сканер `scan/` — обход томов по прямым путям, сравнение с каталогом по времени и размеру файла, чтение тегов через `lofty` (`scan/tags/`, [ADR 0009](adr/0009-tag-library.md)): несколько исполнителей, album artist, сборники, встроенная обложка; запись пачками — треки, артисты, альбомы | Знать об FFI и Kotlin; ссылаться из проекции журнала на каталог внешними ключами |
...
| `core/ffi` (`plinth-ffi`) | Граница с Kotlin на UniFFI: объект `Core` (база, журнал, замок и идентификатор установки в каталоге данных), API по темам — `api/library_api.rs`, `api/journal_api.rs`, `api/scan_api.rs`, `panic::guard`, логи Rust → `CoreLogger` | Паниковать наружу: каждая функция возвращает `Result<_, CoreError>` |
...
- `PlinthCore` — фасад с темами `library` (`CoreLibrary`), `journal`
  (`CoreJournal`) и `scan` (`CoreScan`), типы приложения — `CoreModels.kt`;
