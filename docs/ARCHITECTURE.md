...
| `settings`, `startup` | Настройки и первый запуск: `SortSettings` — порядок списков библиотеки; `startup` — мастер первого запуска (`OnboardingStep`, `OnboardingWizard`, `OnboardingSettings`); всё — в DataStore | — |
...
MainActivity.PlinthApp ── OnboardingViewModel: OnboardingSettings (DataStore) + OnboardingWizard
     ...
     └── пройден или пропущен ──► PlinthMain: вкладки, NavHost, мини-плеер
```
...
каждом шаге; пропущенные шаги записываются — к ним вернутся к месту (F2).
...
Идёт вертикаль «онбординг и старт» (эпик F): готов мастер первого запуска.
