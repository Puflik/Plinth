...
- **Claude сам предупреждает пользователя**, что пора в новый чат, не дожидаясь
  вопроса. Пороги: ~80 тыс. токенов контекста — предупреждение, ~120 тыс. — стоп,
...
- Следит за этим хук `.claude/hooks/chat-budget.py`: считает реальный размер
  контекста по транскрипту, подсказывает на `UserPromptSubmit` и запрещает
  тяжёлые операции в раздутом чате.
