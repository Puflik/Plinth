 ...
 *
 * Порядок задан ключом, а не одним компаратором: ключи сравниваются обычным
 * посимвольным сравнением строк, поэтому тот же порядок получает и база —
 * `ORDER BY` по колонке с ключом, без своей сортировки в SQL.
 *
...
object NaturalOrder {
    ...
    val comparator: Comparator<String> = compareBy(::key)
